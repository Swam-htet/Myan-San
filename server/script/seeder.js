...
async function clearModelData(model) {
  try {
    ...
    console.error(
      `Error clearing data from the ${model.modelName} model:`,
      error
    );
  }
}
...
db.once("open", async () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    let fromTownID = faker.helpers.arrayElement(
      townList.filter((id) => id !== toTownID)
    );
  ...
  // ticket fake data
  for (let i = 0; i < 80; i++) {
    let routeID = faker.helpers.arrayElement(
      [...(await Route.find())].map((route) => route._id)
    );
    ...
    if (randomSeatNumber < availableSeats.length) {
      let selectedSeats = Array.from({ length: randomSeatNumber }, () =>
      ...
      await Ticket.create({
        customer: {
          ...
          NRC: `12/${faker.hacker.noun()}(n)00${faker.helpers.rangeToNumber({
            min: 100,
            max: 3000,
          })}`,
          address: {
